...
{
    [Tracked()]
    [CustomEntity("SpekioToolbox/Sawblade")]
    class Sawblade : Entity
    {
        ...
        private List<float> nodesInfo = new List<float>();
        ...
        public Sawblade(Vector2[] nodes, string mode, string directory, float radius, float totalTime, string stopFlag, string swapFlag, string resumeFlag, string moveFlag, bool drawTrack, int index, int amount, float startOffset, float spacingOffset, string lineColorA, string lineColorB, int id)
        {
            ...
            {
                this.directory = "objects/SpekioToolbox/Sawblade";
            }
        ...
        public override void Added(Scene scene)
        {
            ...
            if (index == 0 && nodes.Count() > 1)
            {
                ...
                {
                   Scene.Add(new Sawblade(nodes, Mode, directory, radius, TotalTime, stopFlag, swapFlag, resumeFlag, moveFlag, false, i, amount, spacingOffset * i, 0, lineColorA, lineColorB, ID));
                }
            }
            UpdatePosition();
        }
        ...
        public override void Update()
        {
            ...
            if (!flagSwapped || Mode == "Flag To Move")
            {
                if (swapped)
                {
                    Percent = Calc.Approach(Percent, 0, Engine.DeltaTime / (nodesInfo[currentNode] * TotalTime / totalLength));
                }
                else
                {
                    Percent = Calc.Approach(Percent, 1, Engine.DeltaTime / (nodesInfo[(currentNode - 1) % nodesInfo.Count] * TotalTime / totalLength));
                }
            }
            else
            {
                ...
                {
                    Percent = Calc.Approach(Percent, 0, Engine.DeltaTime / (nodesInfo[(currentNode - 1) % nodesInfo.Count] * TotalTime / totalLength));
                }
            }
            UpdatePosition();
            if (Percent >= 1f)
